(() => {
  ...
  const toRad = (value) => value * RAD;
  const toDeg = (value) => value * DEG;
  ...
  function getInputs() {
    ...
    return { latitude, inputMinutes, heightRaw, heightKm, longitude, utcOffset, date, terms };
  }

  function inputToSolarMinutes(inputMinutes, inputs) {
    if (!clockMode) return normalizeMinutes(inputMinutes === 1440 ? 0 : inputMinutes);
    return normalizeMinutes(
      inputMinutes + inputs.terms.equationOfTime + 4 * inputs.longitude - 60 * inputs.utcOffset
    );
  }

  function solarToInputMinutes(solarMinutes, inputs) {
    if (!clockMode) return normalizeMinutes(solarMinutes);
    return normalizeMinutes(
      solarMinutes - inputs.terms.equationOfTime - 4 * inputs.longitude + 60 * inputs.utcOffset
    );
  }

  function globePosition(latitude, declination, solarMinutes) {
  ...
  function flatPosition(latitude, declination, solarMinutes, heightKm) {
  ...
  function riseSet(latitude, declination) {
    const phi = toRad(latitude);
    const delta = toRad(declination);
  ...
  function shadowLength(stickHeight, altitude) {
  ...
  function snapshotAtInputMinute(inputMinute, inputs) {
  ...
  function snapshotAtSolarMinute(solarMinute, inputs) {
  ...
  function renderPerspective(snapshot, inputs) {
    ...
    const calibratedDiameter = 2 * noon.slantDistance * Math.tan(toRad(SUN_DIAMETER_DEG / 2));
    const angularSize = toDeg(2 * Math.atan2(calibratedDiameter, 2 * snapshot.flat.slantDistance));
